    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100">My Categories</h2>
                </div>
                <Link
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <!-- Empty State -->
                <div v-if="categories.length === 0" class="rounded-2xl border border-white/60 bg-white/80 px-6 py-16 text-center shadow-sm backdrop-blur-sm dark:bg-[#1e2028]/90 dark:border-[#2e3039]" data-gsap="fade-up">
                    ...
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">No categories yet</p>
                    <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Create your first category to organize products</p>
                    <Link
                        :href="route('vendor.categories.create')"
                ...
                <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <div
                        ...
                        :key="category.id"
                        class="glass-card rounded-2xl border border-white/60 bg-white/80 p-5 shadow-sm backdrop-blur-sm dark:bg-[#1e2028]/90 dark:border-[#2e3039]" data-gsap="fade-up"
                    >
                        <div class="flex items-start justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p v-if="category.parent" class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                                        in {{ category.parent.name }}
                                    </p>
                                    <p v-else class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">Top-level category</p>
                                </div>
                            </div>
                        ...
                        <div class="mt-4 flex items-center gap-3">
                            <span class="rounded-full bg-indigo-50 px-2.5 py-0.5 text-[11px] font-semibold text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">
                                {{ category.products_count }} {{ category.products_count === 1 ? 'product' : 'products' }}
                            </span>
                            <span v-if="category.children.length > 0" class="rounded-full bg-violet-50 px-2.5 py-0.5 text-[11px] font-semibold text-violet-600 dark:bg-violet-500/10 dark:text-violet-400">
                        ...
                        <div v-if="category.children.length > 0" class="mt-3 flex flex-wrap gap-1.5">
                            <span
                                ...
                                :key="child.id"
                                class="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-medium text-gray-500 dark:bg-[#252830] dark:text-gray-400"
                            >
                                {{ child.name }}
                            </span>
                        </div>
